...
import {KurzArbeitVoranmeldung} from './kurzArbeitVoranmeldung';
...
export function generateKurzArbeitVoranmeldung(http: HttpClient, kantonId: number, kantonAmtsstelle: string, anz: number) {
  ...
  for (let i = 0; i < anz; i++) {
    ...
    anfrage.status = faker.random.number({min: 0, max: 3});
    ...
    anfrage.betroffeneAnzMitarbeiter = faker.random.number({min: 0, max: (anfrage.bestandInsgesamt - anfrage.bestandGekuendigt - anfrage.bestandAbruf)});
